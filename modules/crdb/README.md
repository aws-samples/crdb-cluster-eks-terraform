...
|------|-------------|------|---------|:--------:|
| <a name="input_additional_tags"></a> [additional\_tags](#input\_additional\_tags) | Additional tags for resources in AWS | `map(any)` | n/a | "" |
| <a name="input_admission_kv_enabled"></a> [admission\_kv\_enabled](#input\_admission\_kv\_enabled) | When true, work performed by the KV layer is subject to admission control. Check CRDB cluster setting [page](https://www.cockroachlabs.com/docs/stable/cluster-settings) for more information | `string` | `"false"` | no |
| <a name="input_admission_sql_kv_response_enabled"></a> [admission\_sql\_kv\_response\_enabled](#input\_admission\_sql\_kv\_response\_enabled) | when true, work performed by the SQL layer when receiving a KV response is subject to admission control. Check CRDB cluster setting [page](https://www.cockroachlabs.com/docs/stable/cluster-settings) for more information | `string` | `"false"` | no |
| <a name="input_admission_sql_sql_response_enabled"></a> [admission\_sql\_sql\_response\_enabled](#input\_admission\_sql\_sql\_response\_enabled) | when true, work performed by the SQL layer when receiving a DistSQL response is subject to admission control. Check CRDB cluster setting [page](https://www.cockroachlabs.com/docs/stable/cluster-settings) for more information | `string` | `"false"` | no |
| <a name="input_certificate_manifest_directory"></a> [certificate\_manifest\_directory](#input\_certificate\_manifest\_directory) | Directory that contains yaml manifests file to create required certificates. | `string` | `"./manifest/certificate"` | no |     
| <a name="input_client_manifest_file"></a> [client\_manifest\_file](#input\_client\_manifest\_file) | YAML manifests file to create cockroachdb pod/container resources. | `string` | `"./manifest/client-secure-operator.yaml"` | no |
| <a name="input_cluster_organization"></a> [cluster\_organization](#input\_cluster\_organization) | Describes the org name. Check CRDB cluster setting [page](https://www.cockroachlabs.com/docs/stable/cluster-settings) for more information | `string` | `"apg_data_delivery"` | no |
| <a name="input_cluster_settings_file"></a> [cluster\_settings\_file](#input\_cluster\_settings\_file) | File that contains template for for Cluster settings | `string` | `"./manifest/cmd.txt"` | no |
...
| <a name="input_crd_manifest_file"></a> [crd\_manifest\_file](#input\_crd\_manifest\_file) | YAML manifests file to create custom resource. | `string` | `"./manifest/crds.yaml"` | no |
...
| <a name="input_database_manifest_file"></a> [database\_manifest\_file](#input\_database\_manifest\_file) | YAML manifests file to create a crdb database cluster resources. | `string` | `"./manifest/database.yaml"` | no |
...
| <a name="input_eks_cluster_name"></a> [eks\_cluster\_name](#input\_eks\_cluster\_name) | Describes the name of the EKS Cluster | `string` | `""` | yes |
| <a name="input_kv_snapshot_rebalance_max_rate"></a> [kv\_snapshot\_rebalance\_max\_rate](#input\_kv\_snapshot\_rebalance\_max\_rate) | Describes the rate limit (bytes/sec) to use for rebalance and upreplication snapshots. Check CRDB cluster setting [page](https://www.cockroachlabs.com/docs/stable/cluster-settings) for more information | `string` | `"64 MiB"` | no |
| <a name="input_kv_snapshot_recovery_max_rate"></a> [kv\_snapshot\_recovery\_max\_rate](#input\_kv\_snapshot\_recovery\_max\_rate) | Describes the rate limit (bytes/sec) to use for recovery. Check CRDB cluster setting [page](https://www.cockroachlabs.com/docs/stable/cluster-settings) for more information | `string` | `"64 MiB"` | no |
...
| <a name="input_namespace"></a> [namespace](#input\_namespace) | Namespace name for resource creation | `string` | `"staging"` | yes |
| <a name="input_nlb_manifest_file"></a> [nlb\_manifest\_file](#input\_nlb\_manifest\_file) | YAML manifests file to create a network loadbalancer resources. | `string` | `"./manifest/nlb.yaml"` | no |
| <a name="input_node_certificate_file"></a> [node\_certificate\_file](#input\_node\_certificate\_file) | YAML manifest file for creating certificates needed for CRDB node communication | `string` | `".manifest/node_certificate.yaml"` | no |
...
| <a name="input_operator_manifest_directory"></a> [operator\_manifest\_directory](#input\_operator\_manifest\_directory) | Directory that contains yaml manifest files related to CRDB Operators | `string` | `"./manifest/operator"` | no |
